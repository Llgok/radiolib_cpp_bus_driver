 ...
 * @LastEditTime: 2026-04-24 15:57:37
 * @License: GPL 3.0
 */
...
void inline Radiolib_Cpp_Bus_Driver_Hal::pinMode(uint32_t pin, uint32_t mode)
{
    if (pin == RADIOLIB_NC)
    {
        _bus->assert_log(Cpp_Bus_Driver::Tool::Log_Level::INFO, __FILE__, __LINE__, "pinMode fail Invalid argument\n");
        return;
    }
...
void inline Radiolib_Cpp_Bus_Driver_Hal::digitalWrite(uint32_t pin, uint32_t value)
{
    if (pin == RADIOLIB_NC)
    {
        // _bus->assert_log(Cpp_Bus_Driver::Tool::Log_Level::INFO, __FILE__, __LINE__, "Invalid argument\n");
        return;
    }
